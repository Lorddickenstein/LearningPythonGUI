...
def save_all(images):
    # Count the number of png files in the director and add 1 before saving new images
    counter = len(glob.glob(r'extracted-images\*.png')) + 1
    for image in images:
        if image.mode != "RGB":
            image = image.convert("RGB")
        image.save('extracted-images\\img' + str(counter) + ".png", format="png")
        counter += 1

def save_image(image):
    # Count the number of png files in the director and add 1 before saving new image
    counter = len(glob.glob(r'extracted-images\*.png')) + 1
    if image.mode != "RGB":
        image = image.convert("RGB")
    image.save('extracted-images\\img' + str(counter) + ".png", format="png")
...
def open_file():
    ...
    # Open file dialog searching only for .pdf files
    ...
    if file:
        ...
        display_icon('images/arrow_l.png', 2, 0, E, lambda: left_arrow(all_images, current_image, what_text))
        display_icon('images/arrow_r.png', 2, 2, W, lambda: right_arrow(all_images, current_image, what_text))
...
# INSTRUCTIONS
instructions_txt = 'Select a PDF file'
instructions = Label(root, text=instructions_txt, font=('Raleway', 10), bg="white")
instructions.grid(column=2, row=0, sticky=SE, padx=75, pady=5)

# BROWSE BUTTON
browse_txt = StringVar()
browse_btn = Button(root, textvariable=browse_txt, font='Raleway', bg='#20bebe', fg='white', heigh=1, width=15,
